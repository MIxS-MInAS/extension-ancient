# Term: number of reamplification cycles (reamp_pcr_cyc_tot) 


_Number of amplification cycles after library indexing PCR. If capture data, this refers to amplifications prior to the capture experiments._



URI: [MIXS:0001394](https://w3id.org/mixs/0001394)



<!-- no inheritance hierarchy -->


<!--



## Applicable [Checklists](index.md#checklists) and [Extensions](index.md#extensions)

<b>NOTE:</b> does not include Combinations (of checklists and extensions) that use reamp_pcr_cyc_tot.

| Name | Description | Checklist/Extension |
| --- | --- | --- |
| [Ancient](Ancient.md) | A collection of terms appropriate when collecting samples and sequencing samp... | Extension |

-->



## Properties

* Range: [Integer](Integer.md)
* Cardinality: 0..1






## Examples

| Value |
| --- |
| 12 |





## LinkML Source

<details>
```yaml
name: reamp_pcr_cyc_tot
description: Number of amplification cycles after library indexing PCR. If capture
  data, this refers to amplifications prior to the capture experiments.
title: number of reamplification cycles
examples:
- value: '12'
in_subset:
- sequencing
from_schema: https://w3id.org/mixs
rank: 1000
slot_uri: MIXS:0001394
domain_of:
- Ancient
range: integer

```
</details>