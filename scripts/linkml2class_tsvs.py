...
@click.option(
    "--output-dir",
    default="project/class-model-tsvs",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Include parent classes in the output.",
)
@click.option(
    "--eligible-parent-classes",
    multiple=True,
    default=["Checklist", "Extension"],
    help="Eligible parent classes to include in the output.",
)
@click.option(
    "--delete-attributes",
    multiple=True,
    default=["domain_of", "alias", "from_schema", "owner"],
...
def process_schema_classes(
    schema_file: str,
    include_parent_classes: bool,
    eligible_parent_classes: List[str],
    ...
    for metaslot in metaslots:
        ...
        fallback_range = (
            metaslot_obj.range if metaslot_obj.range else metaview.schema.default_range
        )
        ...
        metaslots_helper[metaslot] = {
            ...
            "multivalued": (
                metaslot_obj.multivalued
                if metaslot_obj.multivalued is not None
                else False
            ),
    ...
    for parent_class in eligible_parent_classes:
        current_eligible_leaves = schema_view.class_descendants(
            parent_class, reflexive=include_parent_classes
        )
    ...
    for class_name in sorted_eligible_leaves:
        ...
        sorted_keys = sorted(
            induced_attributes, key=lambda x: induced_attributes[x]["name"]
        )

        # Creating a new OrderedDict that preserves the new order
        sorted_induced_attributes = OrderedDict(
            (k, induced_attributes[k]) for k in sorted_keys
        )

        with open(f"{output_dir}/{class_name}.tsv", "w", newline="") as tsvfile:
            writer = csv.DictWriter(
                tsvfile, fieldnames=(metaslots + annotations), delimiter="\t"
            )
            writer.writeheader()
            ...
            for iak, iav in sorted_induced_attributes.items():
                ...
                for mhk, mhv in metaslots_helper.items():
                    ...
                    if mhk == "examples":
                        ...
                        if iav_mhk_val:
                            structured_pattern_dict = {
                                ...
                                "interpolated": (
                                    iav_mhk_val.interpolated
                                    if iav_mhk_val.interpolated is not None
                                    else False
                                ),
                                "partial_match": (
                                    iav_mhk_val.partial_match
                                    if iav_mhk_val.partial_match is not None
                                    else False
                                ),
                            }
                    ...
                    # Check conditions for metatype and whether it is multivalued
                    elif not mhv["multivalued"] and mhv["metatype"] in [
                    ...
                    elif mhv["multivalued"] and mhv["metatype"] in [
                        "linkml:TypeDefinition",
                        "linkml:ClassDefinition",
                    ]:
                        ...
                        print(
                            f"Unhandled case for {class_name}, {iak}, {mhk} with type {mhv['metatype']}"
                        )
                        temp_dict[mhk] = None
                for annotation_name in annotations:
                    if annotation_name in iav.annotations:
                        ...
                        temp_dict[annotation_name] = iav.annotations[
                            annotation_name
                        ].value
                    else:
